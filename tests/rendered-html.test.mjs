...
test("adds the optional Bible word battle without replacing standard practice", async () => {
  ...
  assert.match(css, /battle-fighter-face-turn 720ms/);
  assert.match(css, /battle-fighter-aim-turn 720ms/);
  assert.match(css, /\.battle-fighter--aim\s*\{[^}]*scaleX\(-1\)/s);
  assert.match(app, /battle-enemy--afterimage/);
  assert.match(app, /battle-impact--echo/);
  assert.match(app, /is-strength-\$\{battleFeedback\.strength\}/);
  assert.match(css, /battle-enemy-afterimage/);
  assert.match(css, /battle-field-impact/);
  assert.match(css, /battle-health-hit/);
  assert.match(css, /battle-depth-projectile 330ms/);
